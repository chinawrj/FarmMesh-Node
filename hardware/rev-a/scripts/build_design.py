#!/usr/bin/env python3
"""Generate the first FarmMesh Node schematic, not a fabrication release."""
...
root.text('FarmMesh Node / Revision A0 - DRAFT',25.4,22.86,3,True)
root.text('1 x RP2350B + 3 x ESP32-C5-WROOM-1U-N8R8\n1S protected Li-ion input; 3.3V regulated rail\nNative KiCad 10 schematic - engineering review only',25.4,35.56,1.8)
...
power.text('RUN nominal: on 3.30V / off 3.01V; tolerances/load sag apply.\nExternal charger/PV is not connected in this draft.',259.08,243.84,1.2)
...
usb.text('USB VBUS is sensed only. It does not power or charge the battery.\nFirmware must gate USB attach on VBUS detection; SWD remains available.\nC5 USB pads are allocated to PARLIO; each C5 uses its UART header.',25.4,38.1,1.5)
...
usb.component('Connector:USB_C_Receptacle_USB2.0_16P','J102','USB4105-GF-A',66.04,111.76,usb_nets,footprint='Connector_USB:USB_C_Receptacle_GCT_USB4105-xx-A_16P_TopMnt_Horizontal',mpn='USB4105-GF-A')
resistor(usb,'R108','5.1k 1%',142.24,88.9,'USB_CC1','GND')
resistor(usb,'R109','5.1k 1%',142.24,114.3,'USB_CC2','GND')
usb.component('Power_Protection:USBLC6-2SC6','D101','USBLC6-2SC6',226.06,114.3,{'1':'USB_DP','6':'USB_DP_ESD','3':'USB_DM','4':'USB_DM_ESD','2':'GND','5':'USB_VBUS'},mpn='USBLC6-2SC6',fields=(248.92,91.44,248.92,93.98))
resistor(usb,'R110','27R at RP',314.96,106.68,'USB_DP_ESD','RP_USB_DP')
resistor(usb,'R111','27R at RP',314.96,132.08,'USB_DM_ESD','RP_USB_DM')
resistor(usb,'R112','5.1k 1%',142.24,172.72,'USB_VBUS','USB_VBUS_SENSE')
resistor(usb,'R113','7.5k 1%',223.52,172.72,'USB_VBUS_SENSE','GND')
capacitor(usb,'C127','100n / 16V',368.3,172.72,'USB_VBUS')
capacitor(usb,'C126','10n',304.8,172.72,'USB_VBUS_SENSE')
usb.component('power:PWR_FLAG','#FLG103','USB_VBUS',111.76,215.9,{'1':'USB_VBUS'})
usb.text('GPIO37 is fail-tolerant: divider 5.1k/7.5k gives 2.60-3.30V at VBUS 4.4-5.5V (1%).\nDisable GPIO37 internal pulls in firmware before testing VBUS.\nUSB differential route: nominal 90 ohm pair, short ESD path,\n27R series parts close to RP. PCB geometry is not designed yet.\nNo USB-to-battery path, no USB-powered standalone operating mode.',25.4,231.14,1.5)
