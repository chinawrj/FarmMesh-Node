...
for ref, m in components.items():
    for num, pin in m['pins'].items():
        ...
        if n is None:
            continue
        global_ = n in {'GND', '+3V3', 'VBAT_PROTECTED', 'PWR_GOOD', 'BAT_SENSE', 'RP_USB_DM', 'RP_USB_DP', 'USB_VBUS_SENSE'} or n.startswith(('A_', 'B_', 'C_')) and not n.endswith('_C5')
...
with (BASE / 'BOM.csv').open('w', newline='') as f:
    ...
    for (value, mpn, fp), refs in sorted(rows.items()):
        w.writerow([', '.join(sorted(refs)), len(refs), value, mpn, fp, 'Draft selected; no stock check' if mpn else 'MPN TBD; footprint provisional'])
