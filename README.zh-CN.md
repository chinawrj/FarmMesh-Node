...
**草案 v0.4 — 产品目标、硬件架构与 A0 原理图**

[English](README.md) | 简体中文

[项目概述](#项目概述) · [硬件首稿](#a0-硬件草案) · [功能与指标](#关键功能与性能目标) · [整体架构](#整体架构) · [内部接口](#候选内部接口) · [无线研究](#无线研究状态) · [待决策事项](#待决策事项) · [阶段规划](#阶段规划) · [联系与参与](#联系与参与) · [参考资料](#参考资料)
...
每个基础设施节点包含 **1 颗 RP2350B 和 3 颗 ESP32-C5**，A0 原理图草案采用三颗官方 **ESP32-C5-WROOM-1U-N8R8 模组**实现 C5 部分。RP2350B 在节点内承担数据汇聚与转发角色，与每颗 C5 分别独立连接。另行保留的**每条板内链路初步目标 50 Mbps** 是硬件设计输入，不是已实现的用户吞吐，也不能据此认定接入服务目标可达；其方向口径及原始速率／有效载荷速率定义仍为 TBD。

项目按**先硬件、后软件**的顺序推进。**首版 A0 原理图草案已可审阅**；PCB、固件、接口时序与性能验证尚未完成。覆盖、用户吞吐、终端容量、功耗和板内链路持续性能均未经实测。

## A0 硬件草案

A0 共七页，包含单节标称 3.7 V 锂电输入及稳压、RP2350B、三颗 C5 模组、候选并行接口、UART 下载排针和 RP USB/SWD 接口。这是用于审阅的首版电路草案，尚非可直接生产的电路板。太阳能充电和储能选型不包含在首版电路中，太阳能部署目标保持不变。

| 交付文件 | 用途与状态 |
| --- | --- |
| [KiCad 工程](hardware/rev-a/FarmMesh-Node.kicad_pro)／[顶层原理图](hardware/rev-a/FarmMesh-Node.kicad_sch) | 可编辑的七页 A0 原理图，包含项目本地库；请一起下载仓库以保留依赖 |
| [七页原理图 PDF](hardware/rev-a/review/FarmMesh-Node-A0.pdf) | A0 原理图审阅副本 |
| [设计说明](hardware/rev-a/DESIGN-NOTES.md) | 电路选择、工作假设、资料来源与后续检查 |
| [C5 审阅记录](hardware/rev-a/reports/c5-review.md)／[电源审阅记录](hardware/rev-a/reports/power-review.md) | 设计复核记录，不是硬件实测 |

C5 模组采用外置天线连接器，原生 USB 管脚分配给 PARLIO，每颗 C5 保留 UART 下载与日志接口。天线、射频布局及接口实际行为仍需验证。**本原理图没有选定无线传输方案或 Mesh 协议。**
...
| 节点组成 | 1 × RP2350B + 3 × ESP32-C5-WROOM-1U-N8R8 | 已选架构组成；首版 A0 原理图可审阅，PCB 尚未完成 |
| A0 硬件文件 | 七页 KiCad 原理图及 PDF；单节标称 3.7 V 锂电输入 | 首版电路草案；太阳能充电、PCB、固件及硬件验证尚未完成 |
| 独立板内链路 | 初步目标：每颗 C5–RP2350 链路 50 Mbps，三路不共享该指标 | 内部接口目标；方向、原始／载荷速率及满足用户服务目标所需的余量 TBD；未验证 |
| 板内接口 | PARLIO ↔ PIO，4 位 TX/RX 分离；两向时钟均由 C5 提供 | 候选方案；每路 12 根信号；A0 已绘制并审阅引脚分配，程序与时序未验证 |
...
**每路共 12 根信号**：8 DATA、2 CLK、2 VALID；三路共 **36 根信号**，另需共同地参考。A0 原理图和 [C5 引脚表](hardware/rev-a/c5-pinmap.json)已记录可供审阅的分配，但尚未冻结或通过硬件验证。
...
以上是资源估计，不代表已有可运行的 PIO 程序或经验证的时序设计。A0 将三路接口分配至 RP2350B GPIO0–35。每个 PIO 有一个 32 GPIO 窗口，通过 GPIOBASE 选择 0 或 16 起点；PIO 配置和指令占用仍须在实现时校核。专用 QSPI 存储引脚、USB、SWD 不占用上述 48 根 Bank0 GPIO，其他板级功能会占用部分剩余 GPIO。具有 30 根 GPIO 的 RP2350A 无法容纳这套 36 根信号方案。
...
| 2 | 原理图与 PCB | 首版 A0 原理图已可审阅；PCB 尚未完成 |
