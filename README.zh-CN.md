...
[项目概述](#项目概述) · [方案范围](#方案范围与当前进展) · [A1 硬件](#a1-硬件原型) · [功能与指标](#关键功能与性能目标) · [整体架构](#整体架构) · [内部接口](#候选内部接口) · [无线研究](#无线研究状态) · [待决策事项](#待决策事项) · [阶段规划](#阶段规划) · [联系与参与](#联系与参与) · [参考资料](#参考资料)

## 项目概述

**FarmMesh Node 是面向农场无线网络的软硬件一体化项目，目标是实现一整套组网方案。** 项目范围包括节点硬件、嵌入式固件、通信协议、Mesh 组网、客户端与传感器接入，以及系统集成和现场验证。软硬件围绕同一套全农场服务目标协同设计；A1 PCB 是整体项目当前的硬件阶段成果。
...
每个基础设施节点包含 **1 颗 RP2350B 和 3 颗 ESP32-C5**，A1 采用三颗官方 **ESP32-C5-WROOM-1U-N8R8 模组**实现 C5 部分。RP2350B 在节点内承担数据汇聚与转发角色，与每颗 C5 分别独立连接。另行保留的**每条板内链路初步目标 50 Mbps** 是硬件设计输入，不是已实现的用户吞吐，也不能据此认定接入服务目标可达；其方向口径及原始速率／有效载荷速率定义仍为 TBD。

开发按节点硬件基础、固件与网络软件、整机及组网验证的阶段推进。**A1 已包含八页原理图、四层 PCB 和可受控工程原型投样的制造包**；放行记录见 [RELEASE.md](hardware/rev-a/RELEASE.md)。固件与网络软件仍待实现。覆盖、用户吞吐、终端容量、功耗和板内链路持续性能均未经实测。

## 方案范围与当前进展

| 组成 | 在整套方案中的职责 | 当前进展 |
| --- | --- | --- |
| 节点硬件 | RP2350B 与三颗 ESP32-C5 模组、电源、PCB、天线及物理接口 | A1 设计和经会审的制造包已交付；待装配及首板调试，部署用太阳能供电仍待设计 |
| 嵌入式固件 | RP2350 PIO/DMA 与 C5 PARLIO 驱动、帧封装、缓冲、流控及三路接口协同 | A1 硬件引脚分配已冻结；固件和接口时序验证待完成 |
| 网络软件 | 节点间无线传输、Mesh 协议与路由、Wi-Fi/BLE 客户端及传感器接入、配置和诊断 | 已记录需求与无线研究；协议选型和实现待完成 |
| 系统集成 | 硬件、固件与网络联调，功耗和射频共存、吞吐、传感器容量及农场部署 | 已有分阶段首板测试计划；多节点和现场验证待推进 |

仓库目前包含硬件发布文件、系统需求及无线研究。软件将在同一项目中继续开发；上表分别说明整体方案范围和已经完成的工作。

## A1 硬件原型

A1 为 100 × 100 mm 四层板，包含受保护单节标称 3.7 V 锂电输入、3.3 V 升降压、RP2350B、三颗 C5 模组、独立并行接口、UART 下载排针及 RP USB/SWD。VBUS 缺失时，硬件开关断开 USB 数据，覆盖 ROM BOOTSEL；USB 不给板供电，也不充电。太阳能充电和储能选型仍为后续工作。

| 交付文件 | 用途与状态 |
| --- | --- |
| [投样放行记录](hardware/rev-a/RELEASE.md) | 当前制造门槛、审阅结论与验证边界 |
| [制造包 ZIP](hardware/rev-a/FarmMesh-Node-A1-manufacturing.zip)／[SHA-256](hardware/rev-a/FarmMesh-Node-A1-manufacturing.zip.sha256) | 已放行受控 A1 原型：Gerber、钻孔、BOM、坐标、图纸、可编辑源文件及会审证据 |
| [KiCad 工程](hardware/rev-a/FarmMesh-Node.kicad_pro)／[PCB](hardware/rev-a/FarmMesh-Node.kicad_pcb) | 可编辑原生文件；请一起下载仓库与项目本地库 |
| [八页原理图 PDF](hardware/rev-a/review/FarmMesh-Node-A1.pdf) | A1 电路审阅副本 |
| [制造说明](hardware/rev-a/FABRICATION.md)／[定料清单](hardware/rev-a/parts-selection.json) | 叠层、装配要求、153 个已定料装配元件及配套件 |
| [首板验收指南](hardware/rev-a/FIRST-BOARD-TEST.md) | 分阶段上电、调试、USB、负载、温升和射频测试；尚未执行实测 |
| [设计说明](hardware/rev-a/DESIGN-NOTES.md) | 电路与布局选择、资料来源与复现方式 |
| [电源审阅](hardware/rev-a/reports/a1-power-review.md)／[RF 与数字接口审阅](hardware/rev-a/reports/a1-rf-digital-review.md)／[DFM 与 BOM 审阅](hardware/rev-a/reports/a1-dfm-bom-review.md) | 独立工程会审记录及首板验证边界 |

C5 模组使用外置双频天线；原生 USB 管脚分配给 PARLIO，每颗 C5 保留 UART 下载与日志。**硬件没有选定无线传输方案或 Mesh 协议。** 制造文件可投样不等于电源、射频或吞吐性能已经验证。
...
| 节点组成 | 1 × RP2350B + 3 × ESP32-C5-WROOM-1U-N8R8 | 已选 A1 架构；尚待实物验证 |
| A1 硬件文件 | 八页 KiCad 原理图、四层 PCB 与定料 BOM；单节标称 3.7 V 锂电输入 | 投样门槛见 RELEASE.md；太阳能充电、固件和实物验证另行推进 |
| 独立板内链路 | 初步目标：每颗 C5–RP2350 链路 50 Mbps，三路不共享该指标 | 内部接口目标；方向、原始／载荷速率及满足用户服务目标所需的余量 TBD；未验证 |
| 板内接口 | PARLIO ↔ PIO，4 位 TX/RX 分离；两向时钟均由 C5 提供 | 候选方案；每路 12 根信号；A1 已绘制并审阅引脚分配，程序与时序未验证 |
...
当前方案为 **ESP32-C5 PARLIO ↔ RP2350 PIO**，每方向 4 位，TX/RX 使用分开的数据线。A1 已冻结硬件连接；同时双向传输的固件机制及时序尚待实现验证。
...
**每路共 12 根信号**：8 DATA、2 CLK、2 VALID；三路共 **36 根信号**，另需共同地参考。A1 原理图和 [C5 引脚表](hardware/rev-a/c5-pinmap.json)已冻结本次原型引脚分配，固件及时序实测仍待完成。
...
以上是资源估计，不代表已有可运行的 PIO 程序或经验证的时序设计。A1 将三路接口分配至 RP2350B GPIO0–35。每个 PIO 有一个 32 GPIO 窗口，通过 GPIOBASE 选择 0 或 16 起点；PIO 配置和指令占用仍须在实现时校核。专用 QSPI 存储引脚、USB、SWD 不占用上述 48 根 Bank0 GPIO，其他板级功能会占用部分剩余 GPIO。具有 30 根 GPIO 的 RP2350A 无法容纳这套 36 根信号方案。
...
这些决策共同服务于完整的软硬件方案。当前仓库已建立 A1 硬件基础并记录软件设计输入；固件、网络协议、路由及系统优化属于接下来的开发阶段。

## 阶段规划

| 阶段 | 范围 | 状态 |
| --- | --- | --- |
| 1 | 系统需求与软硬件架构 | 已记录服务目标及 A1 硬件分工；软件架构与协议待定 |
| 2 | A1 原理图、PCB 与制造包 | 已放行受控工程原型投样，见 RELEASE.md |
| 3 | 首板调试、驱动及板内链路固件 | 待样板装配；固件与时序验证待完成 |
| 4 | 网络软件、Mesh 路由及客户端／传感器服务 | 待开发，与已验证的节点硬件和固件集成 |
| 5 | 多节点联调与农场部署 | 待开展覆盖、吞吐、传感器容量、功耗和可靠性验证 |

## 联系与参与

对 FarmMesh Node 感兴趣？欢迎通过本仓库的 [GitHub Issues](https://github.com/chinawrj/FarmMesh-Node/issues) 交流需求、硬件、固件、网络软件、系统测试或参与方式，也可以访问项目发起人的 [GitHub 主页](https://github.com/chinawrj)。
